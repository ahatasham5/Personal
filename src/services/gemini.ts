...
export const gemini = {
  async summarizeWeek(logs: any[]) {
    const response = await ai.models.generateContent({
      ...
      contents: `Summarize this week's activity based on these logs: ${JSON.stringify(logs)}. 
  ...
  async getBehavioralCoaching(data: any) {
    const response = await ai.models.generateContent({
      ...
      contents: `Based on this user data: ${JSON.stringify(data)}, provide 3 behavioral suggestions. 
  ...
  async getAdaptivePlaybook(data: any) {
    const response = await ai.models.generateContent({
      model: "gemini-3.1-pro-preview",
      contents: `Analyze this user's week: ${JSON.stringify(data)}. 
  ...
  async generateContentIdeas(userData: any, platform: string) {
    const response = await ai.models.generateContent({
      ...
      The ideas MUST be influenced by their specific goals, projects, and daily reflections.
